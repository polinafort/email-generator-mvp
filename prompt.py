...
Не выдумывай факты: даты, проценты, скидки, промокоды, сроки. Если нет данных — ставь плейсхолдеры: [дата], [время], [ссылка], [промокод].
...
Ссылка для вставки (если приложена — используй её; иначе оставляй плейсхолдер):
{final_link}
...
Сгенерируй ОДНУ тему письма — самую удачную (кратко, по делу, без капслока, без «!!!»).
Потом — письмо в формате ниже.

Верни результат строго в таком Markdown-формате:
